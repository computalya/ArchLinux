...
# TO DO		:
#			- htaccess works only first level domains. check for subdomains too
# 			- retype ftp-pass and check pass identity
#			- add ./m.html mobile test page 
...
create_htaccess (){
	LEVEL1=`echo "$DOMAIN_NAME" | cut -d"." -f2`
	LEVEL2=`echo "$DOMAIN_NAME" | cut -d"." -f1`

	echo "RewriteEngine On" &> "${CONTENT_DIR}/.htaccess"
	echo "RewriteCond %{HTTP_HOST} ^www\\.${LEVEL2}\\.${LEVEL1}$ [NC]" &>> "${CONTENT_DIR}/.htaccess"
	echo "RewriteRule ^(.*)$ http://${DOMAIN_NAME}/\$1 [R=301,L]" &>> "${CONTENT_DIR}/.htaccess"
}
